...
def parse_cfg(cfg):
    ...
    cfg.platform = cfg.get('platform', {})
    cfg.platform.accel = cfg.platform.get('accel', 'gpu')
    thermompnn_dir = cfg.platform.get('thermompnn_dir')
    if thermompnn_dir in (None, '', 'auto'):
        cfg.platform.thermompnn_dir = str(resources.files("thermompnn"))
...
def train(cfg):
    ...
    logger = WandbLogger(project=cfg.project, name=cfg.name, log_model=False) if cfg.project is not None else None
    n_steps = 100
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Train ThermoMPNN-D models using two configuration files."
    )
    parser.add_argument(
        "configs",
        metavar="CFG",
        nargs=2,
        help="Two YAML config files (e.g. local.yaml train_single.yaml)",
    )
    return parser
...
def main(argv):
    ...
    cfg = OmegaConf.merge(OmegaConf.load(cfg_paths[0]), OmegaConf.load(cfg_paths[1]))
    train(cfg)
    return 0
